...
DEFAULT_CLIENT_CONFIG_YAML="../ca-client/fabric-ca-client-config.yaml"
...
function enroll {
    ...
    export FABRIC_CA_CLIENT_HOME=`pwd`/../ca-client/client/caserver/admin
...
function start {
    ...
    if [ -f "./fabric-ca-server-config.yaml" ]
    then   
        echo "Using the ./fabric-ca-server-config.yaml"
    else
        echo "No fabric-ca-server-config.yaml found...exiting"
	    return 
    fi
